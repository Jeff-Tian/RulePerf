    ...
    public class WMICmdHelper
    {
        ...
        public string GetServiceStatus(string serviceName)
        {
            ...
            if (!this.ManagementScope.IsConnected)
                this.ManagementScope.Connect();
            ManagementClass services = new ManagementClass(this.ManagementScope, path, null);
            foreach (ManagementObject service in services.GetInstances())
            {
                if (service.GetPropertyValue("Name").ToString().Equals(serviceName, StringComparison.OrdinalIgnoreCase))
                {
                    return service.GetPropertyValue("State").ToString();
                }
            }

            throw new Exception("Service {0} does not exist in machine '{1}'".FormatWith(serviceName, this.machine));
        }
        ...
        {
            this.RestartService(serviceName, TimeSpan.FromMinutes(2));
        }
        ...
        public void RestartServices(string[] services)
        {
            this.RestartServices(services, TimeSpan.FromMinutes(8));
        }
